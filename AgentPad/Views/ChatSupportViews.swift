...
private struct HomeProjectRow: View {
    ...
    private var presentation: (title: String, symbol: String, tint: Color) {
        ...
        case .blocked:
            return ("BLOCKED", "pause.circle.fill", AgentPalette.rose)
        case .completed:
            return ("COMPLETE", "checkmark.seal.fill", AgentPalette.green)
        }
    }
    ...
    private var detail: String {
        let nextStep = project.nextStep.trimmingCharacters(in: .whitespacesAndNewlines)
        if !nextStep.isEmpty, nextStep != "Send the next project request." {
            return nextStep
        }
        let mission = project.mission.trimmingCharacters(in: .whitespacesAndNewlines)
        return mission.isEmpty ? "Ready for its next change." : mission
    }
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 10) {
            Image(systemName: presentation.symbol)
                ...
                .agentControlSurface(
                    radius: 11,
                    tint: presentation.tint.opacity(0.09),
                    selected: false
                )
        ...
        .agentRowSurface(radius: 15, tint: presentation.tint.opacity(0.05), selected: false)
        .accessibilityElement(children: .ignore)
        .accessibilityLabel("\(projectName). \(presentation.title). \(detail). \(changedText).")
