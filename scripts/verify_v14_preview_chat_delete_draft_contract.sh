...
if chat.count(helper_signature) != 1:
    raise SystemExit("ConversationDraftPersistence must exist exactly once")
if chat.count(f'static let storageKey = "{storage_key}"') != 1:
    raise SystemExit("draft invalidation must bind the canonical composer draft storage key")

helper_start = chat.index(helper_signature)
helper_end = chat.index("\nstruct ChatView: View {", helper_start)
helper = chat[helper_start:helper_end]
...
print("PASS: Preview chat deletion purges persisted drafts only after durable success and blocks selected-chat teardown resurrection.")
PY
