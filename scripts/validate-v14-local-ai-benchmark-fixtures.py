...
def validate(root: pathlib.Path) -> ValidationResult:
    ...
    for index, task in enumerate(tasks):
        ...
        if category in REQUIRED_GENERAL_AGENT_CATEGORIES and not task["is_required"]:
            raise ValidationError(f"{label}: #108 required category must have a required task")
