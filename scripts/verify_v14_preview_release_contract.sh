...
# Read only the declaration prefix so switch cases elsewhere cannot masquerade
# as CaseIterable theme definitions.
...
# Launch-only screenshot/test overrides are an active-process presentation
# choice. They must never rewrite the user's durable theme preference.
...
# Keep launch override support alive while enforcing non-persistence. This is
# intentionally implementation-flexible: direct resolution or a dedicated
# resolvedForLaunch helper are both acceptable.
