...
class FixturePackValidationTests(unittest.TestCase):
    ...
    def test_required_category_cannot_be_downgraded(self) -> None:
        ...
        canonical_write(self.root / "manifest.json", manifest)
        with self.assertRaisesRegex(validator.ValidationError, "suite v1 requires true"):
            validator.validate(self.root)

    def test_optional_category_cannot_be_silently_upgraded(self) -> None:
        manifest = self.load_manifest()
        task = next(item for item in manifest["tasks"] if item["category"] == "structuredExtraction")
        task["is_required"] = True
        canonical_write(self.root / "manifest.json", manifest)
        with self.assertRaisesRegex(validator.ValidationError, "suite v1 requires false"):
            validator.validate(self.root)
