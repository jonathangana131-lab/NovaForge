...
struct MissionLayerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            if let completion = director.recentCompletion {
                MissionCompletionBanner(
                    ...
                    onDismiss: director.dismissCompletion
                )
        ...
        .onChange(of: director.isBoardPresented) { _, presented in
            if presented {
                pauseForBoard()
            } else if !session.showGarage && !session.isCrashed {
                session.isPaused = false
            }
        }
        .animation(
            .spring(response: 0.32, dampingFraction: 0.86),
            value: director.isBoardPresented
        )
        .animation(
            .spring(response: 0.34, dampingFraction: 0.80),
            value: director.recentCompletion
        )
    }
    ...
    private var chooseMissionButton: some View {
        Button(action: openBoard) {
            HStack(spacing: 10) {
    ...
    @ViewBuilder
    private func checkpointIcon(for mission: VoltlineMission) -> some View {
        if mission.kind == .route,
           let bearing = director.bearingToNextCheckpoint(session: session) {
    ...
    private func applyQAFixture() {
        let arguments = ProcessInfo.processInfo.arguments
        if arguments.contains("--qa-mission") {
            director.isBoardPresented = true
            pauseForBoard()
        }
        if arguments.contains("--qa-route"),
           director.activeMission == nil,
...
private struct MissionBoardView: View {
        ...
        GeometryReader { proxy in
            ...
            ZStack {
                ...
                .background(
                    LinearGradient(
                        colors: [
                            Color(red: 0.02, green: 0.045, blue: 0.075),
                            .black
                        ],
    ...
    private var summaryRail: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Real movement, speed, purchases, deposits, and photos drive progress.")
            ...
            VStack(alignment: .leading, spacing: 7) {
                Label(
                    "\(director.completionCount) / \(VoltlineMission.catalog.count) finished",
...
private struct MissionCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 9) {
            HStack(alignment: .top) {
                Image(systemName: completed ? "checkmark.seal.fill" : mission.symbol)
                    ...
                    .background(
                        (completed ? Color.green : Color.cyan).opacity(0.11),
                        in: RoundedRectangle(cornerRadius: 11)
                    )
                Spacer()
                Text(
                    mission.reward.formatted(
        ...
        .frame(minHeight: 166)
        .background(
            .white.opacity(active ? 0.085 : 0.052),
            in: RoundedRectangle(cornerRadius: 18)
        )
        .overlay {
            RoundedRectangle(cornerRadius: 18)
                .stroke(
                    active ? .cyan.opacity(0.65) : .white.opacity(0.08),
