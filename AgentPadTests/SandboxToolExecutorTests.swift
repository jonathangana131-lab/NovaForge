...
final class SandboxToolExecutorTests: XCTestCase {
    ...
    func testWriteAndReadTool() throws {
        ...
        XCTAssertTrue(try executor.execute(write).contains("Wrote"))

        let read = ToolRequest(id: "2", name: "read_file", arguments: ["path": "hello.txt"])
        XCTAssertFalse(read.isMutating)
        XCTAssertEqual(try executor.execute(read), "hi")
    }
    ...
    func testRunCommandTool() throws {
        ...
        XCTAssertTrue(try executor.execute(request).contains("Created docs"))
    }
    ...
    func testMutatingFileToolsRejectMissingPayloadsWithoutChangingFiles() throws {
        ...
        XCTAssertThrowsError(try executor.execute(missingWritePayload)) { error in
        ...
        try workspace.write("notes.txt", contents: "original")
        let missingAppendPayload = ToolRequest(
            id: "missing-append-payload",
            name: "append_file",
            arguments: ["path": "notes.txt"]
        )
        XCTAssertThrowsError(try executor.execute(missingAppendPayload)) { error in
            XCTAssertEqual(error as? SandboxError, .invalidArguments)
        }
        XCTAssertEqual(try workspace.read("notes.txt"), "original")

        try workspace.write("replace.txt", contents: "keep TARGET keep")
        let missingReplacementPayload = ToolRequest(
            id: "missing-replacement-payload",
            name: "replace_text",
            arguments: ["path": "replace.txt", "old": "TARGET"]
        )
        XCTAssertThrowsError(try executor.execute(missingReplacementPayload)) { error in
            XCTAssertEqual(error as? SandboxError, .invalidArguments)
        }
    ...
    func testReadRangeAndTailHandleOversizedFilesWithoutWholeRead() throws {
        let contents = (1...30)
            .map { "line-\($0)" }
            .joined(separator: "\n")
        try workspace.write("Logs/big.log", contents: contents)
        ...
        XCTAssertEqual(
            try executor.execute(rangeRequest),
            "10|line-10\n11|line-11\n12|line-12"
        )
        ...
        XCTAssertEqual(
            try executor.execute(tailRequest),
            "28|line-28\n29|line-29\n30|line-30"
        )
    }
    ...
    func testDiffHandlesOversizedFilesWithBoundedComparison() throws {
        ...
        let right = (1...40)
            ...
            .joined(separator: "\n")
        try workspace.write("Logs/left.log", contents: left)
        try workspace.write("Logs/right.log", contents: right)
    ...
    func testReplaceTextStreamsOversizedFileWithoutWholeRead() throws {
        let contents = (1...40)
            ...
            .joined(separator: "\n")
        try workspace.write("Logs/replace.log", contents: contents)
        ...
        XCTAssertEqual(try executor.execute(request), "Replaced 1 occurrence(s) in Logs/replace.log.")
    ...
    func testValidateJSONAndOutlineHandleOversizedFilesWithoutWholeRead() throws {
        ...
        try workspace.write("Data/large.json", contents: oversizedJSON)
        ...
        XCTAssertEqual(try executor.execute(validateRequest), "JSON validation for Data/large.json: ok")
        ...
        let source = (1...40)
            ...
            .joined(separator: "\n")
        try workspace.write("Sources/Large.swift", contents: source)
        ...
        let outline = try executor.execute(outlineRequest)
    ...
    func testCopyToolRefusesToReplaceExistingFolder() throws {
        try workspace.write("Drafts/keep.md", contents: "do not delete")
        try workspace.write("proposal.md", contents: "replacement")
