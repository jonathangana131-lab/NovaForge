    ...
    var body: some View {
        ScrollViewReader { proxy in
            ...
            .onChange(of: model.messages.last?.text.count ?? 0) { _, _ in
                ...
                withAnimation(.smooth(duration: 0.16)) { proxy.scrollTo("bottom", anchor: .bottom) }
            }
            .onChange(of: model.events.count) { _, _ in
                guard model.isGenerating else { return }
                withAnimation(.snappy(duration: 0.18)) { proxy.scrollTo("bottom", anchor: .bottom) }
            }
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
            HStack(spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text(model.currentModelName == "No model loaded" ? "Choose a local model to start" : "Local • private • project-aware")
                        .font(.caption)
            ...
            if model.currentModelName == "No model loaded" {
                Button { model.selectedTab = .models } label: {
                    Label("Open Model Library", systemImage: "arrow.down.circle")
                }
        ...
        ToolbarItem(placement: .topBarLeading) {
            Menu {
                Picker("Agent Mode", selection: $model.agentMode) {
                    ForEach(AppModel.AgentMode.allCases) { mode in
                ...
                Label(model.agentMode.rawValue, systemImage: model.agentMode.symbol)
        ...
        VStack(spacing: 8) {
            if model.agentMode == .plan {
                HStack(spacing: 6) {
                    ...
                    Text("Plan mode is read-only").font(.caption.weight(.medium))
                    Spacer()
                }
                .foregroundStyle(.secondary)
...
private struct GeneratingRow: View {
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            if model.runtimeMetrics.tokensPerSecond > 0 {
                Text(String(format: "%.1f t/s", model.runtimeMetrics.tokensPerSecond))
                    .font(.caption.monospacedDigit())
                    .foregroundStyle(.secondary)
            }
            Text("•").foregroundStyle(.tertiary)
            Text(model.events.last?.title ?? "Planning next action")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .lineLimit(1)
            Spacer()
