...
actor LlamaRuntime {
    ...
    func unload() {
        ...
        vocab = nil; cachedPrompt.removeAll(); modelName = ""
    }
    ...
    func stream(prompt: String, maxTokens: Int = 900) throws -> AsyncThrowingStream<Event, Error> {
        ...
        return AsyncThrowingStream { continuation in
            Task { do { try self.generate(prompt: prompt, maxTokens: maxTokens, continuation: continuation); continuation.finish() } catch { continuation.finish(throwing: error) } }
        }
    }
    ...
    private func generate(prompt: String, maxTokens: Int, continuation: AsyncThrowingStream<Event, Error>.Continuation) throws {
        ...
        let promptTokens = try tokenize(prompt); let lcp = longestCommonPrefix(cachedPrompt, promptTokens)
        ...
        var firstMS = 0.0, generated = 0, position = promptTokens.count; var utf8Bytes = Data()
        while generated < maxTokens && !cancelled {
            let token = llama_sampler_sample(sampler, context, -1); if llama_vocab_is_eog(vocab, token) { break }
            llama_sampler_accept(sampler, token); utf8Bytes.append(contentsOf: tokenBytes(token))
            if let text = String(data: utf8Bytes, encoding: .utf8), !text.isEmpty {
                if generated == 0 { firstMS = seconds(start.duration(to: clock.now)) * 1000 }
                continuation.yield(.text(text)); utf8Bytes.removeAll(keepingCapacity: true)
            }
    ...
    private func decode(tokens: [llama_token], startPosition: Int) throws {
        ...
        var batch = llama_batch_init(Int32(max(tokens.count, 1)), 0, 1); defer { llama_batch_free(batch) }
        for (index, token) in tokens.enumerated() {
            let i = Int(batch.n_tokens); batch.token[i] = token; batch.pos[i] = Int32(startPosition + index); batch.n_seq_id[i] = 1; batch.seq_id[i]![0] = 0; batch.logits[i] = index == tokens.count - 1 ? 1 : 0; batch.n_tokens += 1
        }
    ...
    private func tokenize(_ text: String) throws -> [llama_token] {
        ...
        for _ in 0..<5 {
            ...
            let count = output.withUnsafeMutableBufferPointer { buffer in text.withCString { chars in llama_tokenize(vocab, chars, Int32(byteCount), buffer.baseAddress, Int32(buffer.count), true, true) } }
    ...
    private func tokenBytes(_ token: llama_token) -> Data {
        guard let vocab else { return Data() }; var buffer = Array(repeating: CChar(0), count: 32)
        var count = buffer.withUnsafeMutableBufferPointer { llama_token_to_piece(vocab, token, $0.baseAddress, Int32($0.count), 0, false) }
