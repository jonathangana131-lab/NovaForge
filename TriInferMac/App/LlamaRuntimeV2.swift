...
actor LlamaRuntime {
    enum Profile: String, Sendable {
        ...
        var context: UInt32 { switch self { case .maximumMetal: 4096; case .balanced: 3072; case .extreme27B: 2048 } }
    ...
    enum RuntimeError: LocalizedError { case modelLoad, contextCreate, tokenize, decode(Int32), noModel; var errorDescription: String? { switch self { case .modelLoad: "llama.cpp could not load this GGUF."; case .contextCreate: "llama.cpp could not allocate an inference context. Try a smaller context or quant."; case .tokenize: "Tokenizer failed for this model."; case .decode(let code): "llama.cpp decode failed (\(code))."; case .noModel: "Load a local GGUF first." } } }
    ...
    func load(modelURL: URL, profile: Profile) throws {
        ...
        var mp = llama_model_default_params(); mp.n_gpu_layers = profile.gpuLayers
        ...
        let sp = llama_sampler_chain_default_params(); let chain = llama_sampler_chain_init(sp)
        llama_sampler_chain_add(chain, llama_sampler_init_temp(0.25)); llama_sampler_chain_add(chain, llama_sampler_init_top_p(0.92, 1)); llama_sampler_chain_add(chain, llama_sampler_init_dist(UInt32.random(in: 1...UInt32.max)))
        sampler = chain; cachedPrompt = []; modelName = modelURL.deletingPathExtension().lastPathComponent
    }

    func unload() {
        if let sampler { llama_sampler_free(sampler) }; sampler = nil
        if let context { llama_free(context) }; context = nil
        if let model { llama_model_free(model) }; model = nil; vocab = nil; cachedPrompt.removeAll(); modelName = ""
    }
    ...
    func stream(prompt: String, maxTokens: Int = 900) throws -> AsyncThrowingStream<Event, Error> {
        ...
        return AsyncThrowingStream { continuation in Task { do { try await self.generate(prompt: prompt, maxTokens: maxTokens, continuation: continuation); continuation.finish() } catch { continuation.finish(throwing: error) } } }
    }
    ...
    private func generate(prompt: String, maxTokens: Int, continuation: AsyncThrowingStream<Event, Error>.Continuation) throws {
        ...
        let promptTokens = try tokenize(prompt); let lcp = longestCommonPrefix(cachedPrompt, promptTokens); let memory = llama_get_memory(context)
        if lcp < cachedPrompt.count { _ = llama_memory_seq_rm(memory, 0, Int32(lcp), -1); llama_sampler_reset(sampler) }
        ...
        var firstMS = 0.0, generated = 0, position = promptTokens.count; var utf8: [CChar] = []
        while generated < maxTokens && !cancelled {
            let token = llama_sampler_sample(sampler, context, -1); if llama_vocab_is_eog(vocab, token) { break }; llama_sampler_accept(sampler, token)
            utf8.append(contentsOf: tokenPiece(token))
            if let text = String(validatingUTF8: utf8 + [0]), !text.isEmpty { if generated == 0 { firstMS = durationSeconds(start.duration(to: clock.now)) * 1000 }; continuation.yield(.text(text)); utf8.removeAll(keepingCapacity: true) }
        ...
        if !utf8.isEmpty, let tail = String(validatingUTF8: utf8 + [0]), !tail.isEmpty { continuation.yield(.text(tail)) }
    ...
    private func tokenize(_ text: String) throws -> [llama_token] {
        guard let vocab else { throw RuntimeError.noModel }; let bytes = text.utf8.count; var capacity = max(64, bytes / 2 + 32)
        for _ in 0..<4 {
            var output = Array(repeating: llama_token(0), count: capacity)
            let count: Int32 = output.withUnsafeMutableBufferPointer { buffer in text.withCString { chars in llama_tokenize(vocab, chars, Int32(bytes), buffer.baseAddress, Int32(buffer.count), true, true) } }
            if count >= 0 { return Array(output.prefix(Int(count))) }; capacity = max(capacity * 2, Int(-count) + 8)
        }
        throw RuntimeError.tokenize
    }

    private func tokenPiece(_ token: llama_token) -> [CChar] {
        guard let vocab else { return [] }; var buffer = Array(repeating: CChar(0), count: 32)
        var count: Int32 = buffer.withUnsafeMutableBufferPointer { llama_token_to_piece(vocab, token, $0.baseAddress, Int32($0.count), 0, false) }
    ...
    private func durationSeconds(_ duration: Duration) -> Double { Double(duration.components.seconds) + Double(duration.components.attoseconds) / 1e18 }
    private func metrics(clock: ContinuousClock, start: ContinuousClock.Instant, firstMS: Double, output: Int, prompt: Int, cached: Int) -> Metrics { let seconds = max(0.001, durationSeconds(start.duration(to: clock.now))); return .init(tokensPerSecond: Double(output)/seconds, timeToFirstTokenMS: firstMS, outputTokens: output, promptTokens: prompt, cachedPrefixTokens: cached, backend: "llama.cpp • Metal/CPU • \(profile.rawValue) • \(modelName)") }
}
