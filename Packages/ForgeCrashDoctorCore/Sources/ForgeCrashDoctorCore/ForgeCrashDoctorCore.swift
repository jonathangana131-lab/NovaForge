...
/// Structural defect identity scoped to one host-selected project checkpoint.
/// Project revision is intentionally omitted so the same defect can be recognized across repair revisions.
public struct ForgeCrashRepeatKey: Codable, Hashable, Sendable {
    ...
    var missionSubject: MissionSubject {
        MissionSubject(projectID: projectID, checkpointID: checkpointID)
    }

    struct MissionSubject: Hashable, Sendable {
...
extension ForgeCrashRepeatKey {
    private enum CodingKeys: String, CodingKey {
        case projectID, checkpointID, kind, sourceFile, sourceLine, topStackSymbol, fallbackMessage
    }

    public init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        let projectID = try ForgeCrashLimits.required(
            container.decode(String.self, forKey: .projectID),
            field: "repeat.project",
            maximum: ForgeCrashLimits.identifier
        )
        let checkpointID = try ForgeCrashLimits.required(
            container.decode(String.self, forKey: .checkpointID),
            field: "repeat.checkpoint",
            maximum: ForgeCrashLimits.identifier
        )
        let sourceFile = try ForgeCrashLimits.optional(
            container.decodeIfPresent(String.self, forKey: .sourceFile),
            field: "repeat.sourceFile",
            maximum: ForgeCrashLimits.file
        )
        let sourceLine = try container.decodeIfPresent(Int.self, forKey: .sourceLine)
        if let sourceLine, sourceLine <= 0 {
            throw ForgeCrashValidationError.invalidPositiveInteger(field: "repeat.sourceLine")
        }
        let topStackSymbol = try ForgeCrashLimits.optional(
            container.decodeIfPresent(String.self, forKey: .topStackSymbol),
            field: "repeat.topStackSymbol",
            maximum: ForgeCrashLimits.symbol
        )
        let decodedFallbackMessage = try ForgeCrashLimits.optional(
            container.decodeIfPresent(String.self, forKey: .fallbackMessage),
            field: "repeat.fallbackMessage",
            maximum: ForgeCrashLimits.repeatFallbackMessage
        )
        let fallbackMessage = decodedFallbackMessage.map(Self.normalizeFallbackMessage)
        guard fallbackMessage == decodedFallbackMessage else {
            throw ForgeCrashValidationError.invalidRepeatKey
        }
        let hasStructuralIdentity = sourceFile != nil || sourceLine != nil || topStackSymbol != nil
        guard hasStructuralIdentity != (fallbackMessage != nil) else {
            throw ForgeCrashValidationError.invalidRepeatKey
        }
        ...
        self.init(
            ...
            kind: try container.decode(ForgeCrashIncidentKind.self, forKey: .kind),
            sourceFile: sourceFile,
            sourceLine: sourceLine,
            topStackSymbol: topStackSymbol,
...
public enum ForgeCrashTriage {
    public static func makeSubmission(
        ...
        let subjectFailures = failedHistory.attempts.lazy.filter {
            $0.repeatKey.missionSubject == repeatKey.missionSubject
        }
        let totalFailures = subjectFailures.count
        let repeatedFailures = subjectFailures.filter { $0.repeatKey == repeatKey }.count
