...
final class ForgeQualityStaticTrustBoundaryTests: XCTestCase {
    func testExternalConsumerCannotMintTrustedPolicyRunMeasurementOrBatch() throws {
        ...
        func attemptBatch(_ batch: ForgeQualityMeasurementBatch) {
        ...
        XCTAssertTrue(
            diagnostics.contains("ForgeQualityTrustedMeasurementBatch"),
            "Expected a whole-batch trust rejection, got: \(diagnostics)"
        )
        XCTAssertTrue(
