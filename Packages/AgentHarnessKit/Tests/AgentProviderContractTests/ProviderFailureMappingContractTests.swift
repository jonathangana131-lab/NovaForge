...
final class ProviderFailureMappingContractTests: XCTestCase {
    ...
    func testLocalizedErrorUsesSafePublicMessage() {
        let failure = ProviderFailureMapper.httpFailure(
            statusCode: 401,
            providerID: providerID,
            adapterID: adapterID
        ...
        XCTAssertEqual(failure.errorDescription, failure.publicMessage)
        XCTAssertTrue(failure.publicMessage.contains("key or login"))
    }
}
