...
public extension ToolDescriptor {
    /// SHA-256 of canonical sorted-key JSON. The tool identity is bound separately by
    /// `ApprovalBinding`, so aliases and display metadata cannot perturb this digest.
    func canonicalArgumentDigest(for arguments: JSONValue) throws -> String {
        try ToolArgumentValidator.validate(arguments, against: argumentSchema)
        let digest = ToolSHA256.digest(try AgentToolJSON.data(for: arguments))
        return "sha256:" + digest.map { String(format: "%02x", $0) }.joined()
    }
}

private enum ToolSHA256 {
