...
public enum ForgePerformanceError: Error, Equatable, Sendable {
    ...
    case untrustedMeasurementBatchReceipt(String)
...
public struct ForgePerformanceEnvironment: Hashable, Codable, Sendable {
    ...
    public init(kind: ForgePerformanceEnvironmentKind, hardwareIdentifier: String, osVersion: String, osBuild: String) throws {
        self.kind = kind
        self.hardwareIdentifier = try ForgePerformanceValidation.identifier(hardwareIdentifier, field: "environment.hardwareIdentifier", maximum: 256)
        self.osVersion = try ForgePerformanceValidation.identifier(osVersion, field: "environment.osVersion", maximum: 128)
        self.osBuild = try ForgePerformanceValidation.identifier(osBuild, field: "environment.osBuild", maximum: 128)
    }

    private enum CodingKeys: String, CodingKey { case kind, hardwareIdentifier, osVersion, osBuild }
...
/// Exact product/runtime/device/protocol identity shared by the accepted performance budget and measurements.
/// The receipt field is candidate metadata until the package receives a non-persistable host trust binding.
public struct ForgePerformanceTarget: Hashable, Codable, Sendable {
    ...
    public init(
        ...
        self.measurementProtocolID = try ForgePerformanceValidation.identifier(measurementProtocolID, field: "target.measurementProtocolID", maximum: 256)
        self.measurementProtocolRevision = try ForgePerformanceValidation.positiveRevision(measurementProtocolRevision, field: "target.measurementProtocolRevision")
        self.budgetRevision = try ForgePerformanceValidation.positiveRevision(budgetRevision, field: "target.budgetRevision")
        self.budgetAuthorityReceiptID = try ForgePerformanceValidation.identifier(budgetAuthorityReceiptID, field: "target.budgetAuthorityReceiptID")
    }
...
/// V1 performance constraints are conservative upper bounds, so lower measured values are better.
...
public struct ForgePerformanceBudget: Hashable, Codable, Sendable {
    ...
    public init(
        ...
        guard schemaVersion == Self.currentSchemaVersion else { throw ForgePerformanceError.unsupportedSchema(schemaVersion) }
        ...
        try ForgePerformanceValidation.boundedCount(constraints.count, field: "budget.constraints", maximum: Self.maximumConstraints)
...
public struct ForgePerformanceObservation: Hashable, Codable, Sendable {
    ...
    public init(metric: ForgePerformanceMetric, value: Double, sampleCount: Int) throws {
        guard value.isFinite, value >= 0 else { throw ForgePerformanceError.invalidObservation(metric.rawValue) }
        switch metric.unit {
        case .percent:
            guard value <= 100 else { throw ForgePerformanceError.invalidObservation(metric.rawValue) }
        case .bytes, .count:
            guard value.rounded(.towardZero) == value else { throw ForgePerformanceError.invalidObservation(metric.rawValue) }
        case .milliseconds, .milliwattHours:
            break
        }
...
public struct ForgePerformanceRun: Hashable, Codable, Sendable {
    ...
    public init(runID: String, target: ForgePerformanceTarget, observations: [ForgePerformanceObservation]) throws {
        ...
        try ForgePerformanceValidation.boundedCount(observations.count, field: "run.observations", maximum: Self.maximumObservations)
        var seen = Set<ForgePerformanceMetric>()
...
/// A measurement batch is an indivisible candidate subject. A future canonical producer adapter must
/// authenticate that this batch covers the complete accepted run set; callers cannot mint that trust.
public struct ForgePerformanceMeasurementBatch: Hashable, Codable, Sendable {
    ...
    public init(
        ...
        guard schemaVersion == Self.currentSchemaVersion else { throw ForgePerformanceError.unsupportedSchema(schemaVersion) }
        self.batchReceiptID = try ForgePerformanceValidation.identifier(batchReceiptID, field: "batch.batchReceiptID")
        ...
        var seen = Set<String>()
        for run in runs {
            guard seen.insert(run.runID).inserted else { throw ForgePerformanceError.duplicateRunID(run.runID) }
...
/// Non-persistable host trust over the complete validated budget subject.
/// The initializer is module-internal so persisted/model-authored bytes cannot mint authority.
public struct ForgePerformanceBudgetTrustBinding: Equatable, Sendable {
    private let authenticatedBudget: ForgePerformanceBudget

    public var budgetAuthorityReceiptID: String { authenticatedBudget.target.budgetAuthorityReceiptID }
    public var target: ForgePerformanceTarget { authenticatedBudget.target }

    init(authenticatedBudget: ForgePerformanceBudget) {
        self.authenticatedBudget = authenticatedBudget
    }

    func exactlyMatches(_ budget: ForgePerformanceBudget) -> Bool {
        authenticatedBudget == budget
    }
}

/// Non-persistable host trust over the complete validated measurement batch subject.
/// The initializer is module-internal so a reused receipt ID cannot authenticate altered observations.
public struct ForgePerformanceMeasurementTrustBinding: Equatable, Sendable {
    private let authenticatedBatch: ForgePerformanceMeasurementBatch

    public var batchReceiptID: String { authenticatedBatch.batchReceiptID }
    public var target: ForgePerformanceTarget { authenticatedBatch.target }

    init(authenticatedBatch: ForgePerformanceMeasurementBatch) {
        self.authenticatedBatch = authenticatedBatch
    }

    func exactlyMatches(_ batch: ForgePerformanceMeasurementBatch) -> Bool {
        authenticatedBatch == batch
    }
}
...
/// Derived-only evaluator output. Relaunch/restore must re-evaluate current raw evidence with fresh host trust.
...
public enum ForgePerformanceEvaluator {
    public static func evaluate(
        ...
        budgetTrust: ForgePerformanceBudgetTrustBinding,
        measurementTrust: ForgePerformanceMeasurementTrustBinding
        ...
        guard budgetTrust.exactlyMatches(budget) else {
            throw ForgePerformanceError.untrustedBudgetAuthorityReceipt(budget.target.budgetAuthorityReceiptID)
        }
        guard measurementTrust.exactlyMatches(batch) else {
            throw ForgePerformanceError.untrustedMeasurementBatchReceipt(batch.batchReceiptID)
        }
    ...
    private static func blockerSort(_ lhs: ForgePerformanceBlocker, _ rhs: ForgePerformanceBlocker) -> Bool {
        func key(_ blocker: ForgePerformanceBlocker) -> String {
            switch blocker {
            case let .insufficientRuns(required, actual): return "0|\(required)|\(actual)"
            case let .missingMetric(metric, runID): return "1|\(metric.rawValue)|\(runID)"
...
/// Persist only validated raw inputs. Accepted/failed performance verdicts and trust bindings are never serialized.
public struct ForgePerformanceEvidenceArchive: Codable, Sendable {
    ...
    public init(schemaVersion: Int = Self.currentSchemaVersion, budget: ForgePerformanceBudget, batch: ForgePerformanceMeasurementBatch) throws {
        guard schemaVersion == Self.currentSchemaVersion else { throw ForgePerformanceError.unsupportedSchema(schemaVersion) }
