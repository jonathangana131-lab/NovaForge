...
public enum ForgeCompactGovernor {
    ...
    /// Keep governor identity semantics aligned with the Project Capsule/cache boundary: exact
    /// canonical spelling, bounded UTF-8 size, and no control characters. The governor consumes
    /// evidence identities; it must not normalize or widen them on the way into local execution.
