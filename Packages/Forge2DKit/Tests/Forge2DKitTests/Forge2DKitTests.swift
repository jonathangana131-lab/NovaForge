...
final class Forge2DKitTests: XCTestCase {
    ...
    func testDefaultBlueprintGeneratesDeterministicSelfContainedProject() throws {
        ...
        XCTAssertEqual(first.semanticAutomationCapabilities, [.activateControl, .setActionValue])
    }
    ...
    func testGeneratedProjectOptsIntoCanonicalSemanticAutomationContract() throws {
        let project = try Forge2DGenerator.generate(blueprint)
        let html = try XCTUnwrap(project.files.first(where: { $0.path == "index.html" })?.contents)
        let js = try XCTUnwrap(project.files.first(where: { $0.path == "game.js" })?.contents)

        XCTAssertTrue(html.contains("data-novaforge-control=\"pause\""))
        XCTAssertTrue(html.contains("data-novaforge-action=\"move-horizontal\""))
        XCTAssertTrue(html.contains("data-novaforge-action=\"jump\""))
        XCTAssertTrue(js.contains("addEventListener(\"novaforge:action\""))
        XCTAssertTrue(js.contains("semanticActionValue(event, \"move-horizontal\")"))
        XCTAssertTrue(js.contains("semanticActionValue(event, \"jump\")"))
        XCTAssertTrue(js.contains("input.semanticAxis = value"))
        XCTAssertTrue(js.contains("jumpDown && !input.semanticJumpWasDown"))
    }

    func testSemanticMovementStaysBoundedAndCombinesWithExistingInputSources() throws {
        let js = try XCTUnwrap(try Forge2DGenerator.generate(blueprint).files.first(where: { $0.path == "game.js" })?.contents)

        XCTAssertTrue(js.contains("return bounded(detail.value, -1, 1)"))
        XCTAssertTrue(js.contains("digitalHorizontal + input.gamepadAxis + input.semanticAxis"))
        XCTAssertTrue(js.contains("bounded(digitalHorizontal + input.gamepadAxis + input.semanticAxis, -1, 1)"))
    }
