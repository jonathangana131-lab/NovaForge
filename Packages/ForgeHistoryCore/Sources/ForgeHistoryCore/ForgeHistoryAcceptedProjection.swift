...
/// Opaque identity of the accepted project state proven by an upstream Mission/ProjectStore
/// checkpoint. History preserves the normalized upstream value without reinterpreting it as a
/// filesystem path, provider receipt, runtime grant, or permission.
...
/// checkpoint/project stores plus `ForgeHistoryCheckpoint`. This wrapper deliberately is not
/// `Codable`, so persistence cannot decode a second History acceptance authority.
...
public enum ForgeHistoryAcceptedTimelineProjector {
    public static func project(
        ...
        // Keep ForgeHistoryTimeline as the single authority for canonical ordering, lineage,
        // sequence uniqueness, parent chronology, and optional mission scoping.
        ...
        for checkpoint in timeline.checkpoints {
            guard let acceptedProjectStateID = projectStateByCheckpoint[checkpoint.id] else {
                // This should be unreachable after duplicate/project validation, but remains a
                // fail-closed guard if canonical timeline construction ever changes.
