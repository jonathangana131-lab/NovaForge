...
final class ForgeHistoryAcceptedProjectionTests: XCTestCase {
    ...
    func testProjectorRetainsAcceptedProjectStateIdentityInCanonicalTimelineOrder() throws {
        ...
        XCTAssertEqual(
            projection.acceptedProjectStates.map(\.checkpointID.rawValue),
            ["c1", "c2"]
        )
        XCTAssertEqual(
    ...
    func testAcceptedProjectStateIdentityMatchesMissionNormalizationAndRemainsOpaque() throws {
        let stateID = try ForgeHistoryAcceptedProjectStateID("  /tmp/project state#1  ")
        XCTAssertEqual(stateID.rawValue, "/tmp/project state#1")
    }

    func testAcceptedProjectStateIdentityRejectsBlankValue() {
    ...
    func testLookupDoesNotInventBindingForUnknownCheckpoint() throws {
        let project = try ForgeHistoryProjectID("project-a")
        let checkpoint = try makeCheckpoint("c1", sequence: 1)
        let projection = try ForgeHistoryAcceptedTimelineProjector.project(
            projectID: project,
        ...
        XCTAssertNil(
            projection.acceptedProjectStateID(for: try ForgeHistoryCheckpointID("unknown"))
        )
    }
    ...
    private func makeCheckpoint(
        ...
        try ForgeHistoryCheckpoint(
            id: ForgeHistoryCheckpointID(id),
            parentID: parent,
            originatingMissionID: missionID,
            sequence: sequence,
            acceptedAtMilliseconds: 1_000 + Int64(sequence),
            title: "Checkpoint \(id)"
        )
    }
}
